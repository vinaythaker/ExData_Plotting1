...
# Create Plot4
par(mfrow=c(2,2), font.lab=1, cex.lab=0.8, cex.axis=0.8, family="sans")
plot(x=dt2$DateTime, y=dt2$Global_active_power, type="l", xlab="", ylab="Global Active Power (kilowatts)")

plot(x=dt2$DateTime, y=dt2$Voltage, type="l", xlab="datetime", ylab="Voltage")

plot(x=dt2$DateTime, y=dt2$Sub_metering_1, col="black", type="l", xlab="", ylab="Energy sub metering", ylim=c(0,40))
lines(x=dt2$DateTime, y=dt2$Sub_metering_2, col="red")
lines(x=dt2$DateTime, y=dt2$Sub_metering_3, col="blue")
legend(x="topright", legend=c("Sub_metering_1","Sub_metering_2", "Sub_metering_3"),  lty=c(1,1,1),lwd=c(1,1,1), col=c("black","blue","red"), cex=0.8, yjust=5) 

plot(x=dt2$DateTime, y=dt2$Global_reactive_power, type="l", xlab="datetime", ylab="Global_reactive_power")
